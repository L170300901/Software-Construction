...
public class ConcreteCircularOrbit<L,E> implements CircularOrbit<L,E>{
	...
	@Override
	public void setTrack(Element element) {//전자를 담을 트랙만 생성해놓는다.
	...
	@Override
	public void removeTrack(Element element) {//모든 트랙을 삭제한다.
		// TODO Auto-generated method stub
		if(!track.isEmpty()) {
    ...
    @Override
	public void setElement(Element element) {//트랙에 element를 설정한다  
    	// TODO Auto-generated method stub
    	if(!track.isEmpty()) {
    		for(int i=0; i<element.electon.size(); i++) {
    			...
    			for(int j=0; j<element.electon.get(i).size(); j++) {
    				element.electon.get(i).get(j).onTrack = true; //트랙위에 올라갔을 때 onTrack의 값이 true로 됨
    			}
    		}
